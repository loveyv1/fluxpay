...
fn test_initialize() {
    ...
    let contract_id = env.register_contract(None, RexToken);
    let client = RexTokenClient::new(&env, &contract_id);

    client.initialize(
        &admin,
        &String::from_str(&env, "FluxSwap"),
        &String::from_str(&env, "REX"),
        &9,
    );

    assert_eq!(client.name(), String::from_str(&env, "FluxSwap"));
    assert_eq!(client.symbol(), String::from_str(&env, "REX"));
