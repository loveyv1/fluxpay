...
export const useFreighter = () => {
  ...
  const checkSession = useCallback(async () => {
    ...
    if (typeof window !== 'undefined' && localStorage.getItem('fluxswap_disconnected') === 'true') {
      return;
    }
  ...
  const connect = useCallback(async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('fluxswap_disconnected');
    }
  ...
  const disconnect = useCallback(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('fluxswap_disconnected', 'true');
    }
    setPublicKey('');
    setConnected(false);
    setError(null);
