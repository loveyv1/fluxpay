 ...
 * ops/deploy.js
 *
 * Full FluxSwap deployment script.
 * Covers: fund → deploy → initialize → mint → add_liquidity → save record.
 *
 * Usage:
 *   STELLAR_ISSUER_SECRET=S... STELLAR_DISTRIBUTOR_SECRET=S... node ops/deploy.js
 */
...
// ─── STEP 2 — Mint Classic REX via Trustline Script ────────────────────────
async function step2_mint_classic() {
  console.log('\n═══ STEP 2 — Mint Classic REX ═══');
  log('Running setup-trustlines.js to mint classic REX to Distributor…');
  try {
    // Just run the external script which does changeTrust + Payment
    const output = run('node ops/setup-trustlines.js');
    console.log(output);
    ok('Classic REX Minted successfully.');
  } catch (e) {
...
function step3_deploy() {
  ...
  // Deploy SAC for REX
  log(`Deploying SAC Wrapper for REX:${ISSUER_PUB}…`);
  let rexId;
  try {
    rexId = soroban(`contract asset deploy --asset REX:${ISSUER_PUB} --source-account ${ISSUER_SECRET}`);
    ok(`REX SAC deployed → ${rexId}`);
  } catch (e) {
    if (e.message.includes('ExistingValue')) {
      rexId = soroban(`contract id asset --asset REX:${ISSUER_PUB}`).trim();
      ok(`REX SAC already exists → ${rexId}`);
    } else {
...
function step4_initialize(rexId, poolId, bridgeId) {
  console.log('\n═══ STEP 4 — Initialize contracts ═══');
  const hashes = {};

  // REX Token (SAC Wrapper) doesn't need initialization!
  hashes.rexInit = 'SAC wrappers do not require initialization';
  ok('REX SAC ready.');
  ...
  try {
    ...
    hashes.poolInit = invoke(poolId, ISSUER_SECRET, 'initialize',
      `--token_contract ${rexId} --xlm_contract ${XLM_CONTRACT_ID} --admin ${ISSUER_PUB}`
    );
  ...
  try {
    hashes.bridgeInit = invoke(bridgeId, ISSUER_SECRET, 'initialize',
      `--token_contract ${rexId} --pool_contract ${poolId} --admin ${ISSUER_PUB}`
    );
...
function step5_liquidity(poolId) {
  console.log('\n═══ STEP 5 — Add initial liquidity ═══');
  // 100,000 REX (×10^7) and 4,000 XLM (×10^7)
  const TOKEN_AMT = '1000000000000';  // 100,000 REX
  const XLM_AMT   = '40000000000';    // 4,000 XLM

  log(`Adding liquidity: ${TOKEN_AMT} REX stroops + ${XLM_AMT} XLM stroops…`);
  try {
...
function step6_save(data) {
  ...
  const record = {
    ...
    rexAsset: `REX:${ISSUER_PUB}`,
    REXToken: {
      contractId: data.rexId,
      wasmHash: data.rexWasmHash || '',
      initTxHash: data.rexInit || '',
    },
    LiquidityPool: {
...
async function main() {
  console.log('\n╔════════════════════════════════════════════╗');
  console.log('║   FluxSwap — Full Deploy                     ║');
  console.log('╚════════════════════════════════════════════╝');
  console.log(`\n  Issuer:      ${ISSUER_PUB}`);
  ...
  const { rexId, poolId, bridgeId } = step3_deploy();
  const { rexInit, poolInit, bridgeInit } = step4_initialize(rexId, poolId, bridgeId);
  const lpTxHash    = step5_liquidity(poolId);

  const record = step6_save({
    rexId, poolId, bridgeId,
    rexInit, poolInit, bridgeInit,
  ...
  console.log('╠════════════════════════════════════════════╣');
  console.log(`║  REX Token: ${record.REXToken.contractId.padEnd(30)} ║`);
  ...
  console.log('  2. Run: node ops/setup-trustlines.js');
